...
class thk_qr_code extends WP_Widget {
	public function __construct() {
		...
		parent::__construct( 'qr', '#8 ' . __( 'QR Code', 'luxeritas' ) . ' (' . __( 'by Luxeritas', 'luxeritas' ) . ')', $widget_ops );
	}
...
class thk_swiper_widget extends WP_Widget {
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['ids']	= isset( $new_instance['ids'] )		? sanitize_text_field( $new_instance['ids'] )   : null;
	...
	public function form( $instance ) {
		...
		$ids	 = isset( $instance['ids'] )		? sanitize_text_field( $instance['ids'] )	: $this->_defaults['ids'];
 ...
 * ウィジェットの WAF 対策
 *---------------------------------------------------------------------------*/
if( is_admin() === true && isset( $luxe['measures_against_waf'] ) && !empty( $_POST ) ) {
...
add_action( 'widgets_init', function() {
	...
	foreach( $widgets as $value ) register_sidebars( 1, $value );
	unset( $widgets );
	...
	register_widget( 'thk_swiper_widget' );
	register_widget( 'thk_toc_widget' );
} );
