...
function thk_custom_css() {
	...
	if( isset( $luxe['luxe_mode_select'] ) && $luxe['luxe_mode_select'] === 'bootstrap' ) {
		//$style['min_576']  .= '.container{width: 540px;}';
		$style['min_768']  .= '.container{width: 720px;}';
		$style['min_992']  .= '.container{width: 960px;}';
		$style['min_1200'] .= '.container{width: 1140px;}';
	}
...
GNAVI_SMART_ICON;
	...
	/* グローバルナビ中央寄せ */
	if( $luxe['global_navi_auto_resize'] === 'auto' && isset( $luxe['global_navi_center'] ) ) {
		$style['min_992'] .= '#gnavi > div{display:table;margin-right:auto;margin-left:auto;}';
	}

	/* グローバルナビに区切り線を追加 */
	if( $luxe['global_navi_sep'] !== $defaults['global_navi_sep'] ) {
		...
		if( $luxe['global_navi_auto_resize'] === 'same' ) {
	...
	/* グローバルナビ自動リサイズ */
	if( $luxe['global_navi_auto_resize'] === 'full' ) {
...
GNAVI_AUTO;
	...
	/* グローバルナビ全幅同じ */
	elseif( $luxe['global_navi_auto_resize'] === 'same' ) {
		$style['min_992'] .= <<<GNAVI_AUTO
#gnavi ul {
	display: table;
	table-layout: fixed;
...
GNAVI_AUTO;
	...
	if( $luxe['global_navi_position'] !== $defaults['global_navi_position'] ) {
		$top = $luxe['head_band_height'] - 1;
...
BODY_BEFORE;
	 ...
	 * サムネイルの大きさ
	 *---------------------------------------------------------------------------*/
	if( isset( $luxe['thumbnail_is_size'] ) && $luxe['thumbnail_is_size'] !== 'generate' ) {	// WordPress のデフォルトのサムネイルを使う場合
		if( $luxe['thumbnail_is_size'] !== 'thumbnail' ) {
			...
			*/
			if( $luxe['thumbnail_is_size'] === 'medium' ) {
/*
				$style['576_991'] .= '#list .term img{width:150px;height:auto;}';	// PC:300px Mobile:150px
				$style['max_575'] .= '#list .term img{width:100px;height:auto;}';	// Small device:100px
*/
				$style['all'] .= 'max-width:300px;height:auto;';	// PC:150px Mobile:100px
				$style['max_1199'] .= '#list .term img{max-width:200px;height:auto;}';	// PC:300px Mobile:200px
				$style['max_575'] .= '#list .term img{max-width:100%;height:auto;float:none}';	// Small device:300p(float:none)
			}
			else {
				$style['all'] .= 'max-width:100%;width:auto;height:auto;';	// 大きいサムネイル
			}
			$style['all'] .= '}';

			if( $luxe['thumbnail_is_size'] === 'large' || $luxe['thumbnail_is_size'] === 'full' ) {	// 大きいサムネイルの時は小デバイスで float 解除
				$style['max_575'] .= '#list .term img{float:none;}';
			}

			$style['min_1200'] .= '#list .term img{width:auto;height:auto;}';	// PC の時は常に auto (サムネイルかどうかの判断は WordPress側に任せる)
		}
	}
	else {	// カスタマイズ画面で入力した値でサムネイルを作成する場合
		if(
			( $luxe['thumbnail_width'] > 0 && $luxe['thumbnail_width'] !== $defaults['thumbnail_width'] ) ||
			( $luxe['thumbnail_height'] > 0 && $luxe['thumbnail_view_height'] !== $defaults['thumbnail_view_height'] )
		) {
			$style['all'] .= '.term img, #list div[id^="tile-"] .term img, #list div[id^="card-"] .term img{';

			if( $luxe['thumbnail_width'] > 0 && $luxe['thumbnail_width'] !== $defaults['thumbnail_width'] ) {
				$small_thumb_w = (int)ceil( $luxe['thumbnail_width'] * 0.666 );
				$style['all'] .= 'width:' . $small_thumb_w . 'px;';
			}
			if( $luxe['thumbnail_height'] > 0 && $luxe['thumbnail_view_height'] !== 'auto' ) {
				$small_thumb_h = (int)ceil( $luxe['thumbnail_height'] * 0.666 );
				$style['all'] .= 'height:' . $small_thumb_h . 'px;';
			}
			$style['all'] .= '}';

			$style['min_576'] .= '#list .term img, #list div[id^="tile-"] .term img, #list div[id^="card-"] .term img{';
			if( $luxe['thumbnail_width'] > 0 && $luxe['thumbnail_width'] !== $defaults['thumbnail_width'] ) {
				$style['min_576'] .= 'width:' . $luxe['thumbnail_width'] . 'px;';
			}
			if( $luxe['thumbnail_height'] > 0 && $luxe['thumbnail_view_height'] !== 'auto' ) {
				$style['min_576'] .= 'height:' . $luxe['thumbnail_height'] . 'px;';
			}
			else {
				$style['min_576'] .= 'height:' . 'auto;';
			}
			$style['min_576'] .= '}';
		}
	}

	// タイル型の幅いっぱい・中央揃え(スマホサイズでは解除)
	if( isset( $luxe['grid_enable'] ) ) {
		if( !isset( $luxe['thumbnail_tile_width_full'] ) || isset( $luxe['thumbnail_tile_align_center'] ) ) {
			// タイル型の幅いっぱい
			$style['min_576'] .= '#list div[id^="tile-"] .term img{';
			if( isset( $luxe['thumbnail_tile_width_full'] ) ) {
				$style['min_576'] .= 'width:100%;';
			}
			// タイル型の中央揃え
			if( isset( $luxe['thumbnail_tile_align_center'] ) ) {
	...
	if( $luxe['thumbnail_layout'] === 'flow' ) {
		$style['min_576'] .= '#list .excerpt{overflow:visible;}';
	}
	elseif( $luxe['thumbnail_layout'] === 'under' ) {
		$style['all'] .= '#list .term img{float:none;margin-right:0}';
	}

	if( isset( $luxe['thumbnail_is_size_card'] ) ) {
		if( $luxe['thumbnail_is_size_card'] === 'thumbnail' ) {
			$style['max_575'] .= 'div[id^="card-"] h2{clear:left;}';
		}
		elseif( $luxe['thumbnail_is_size_card'] === 'medium' ) {
			$style['max_575'] .= 'div[id^="card-"] h2{clear:left;}';
		}
	}
	...
	if( isset( $luxe['head_band_visible'] ) ) {
		...
		if(
			...
			if( isset( $luxe['head_band_follow_color'] ) ) {
				...
				if( isset( $luxe['head_band_twitter'] ) )	$style['all'] .= 'div[id*="head-band"] .twitter a{background:#55acee;}';
				if( isset( $luxe['head_band_facebook'] ) )	$style['all'] .= 'div[id*="head-band"] .facebook a{background:#3b5998;}';
				if( isset( $luxe['head_band_hatena'] ) )	$style['all'] .= 'div[id*="head-band"] .hatena a{background:#3c7dd1;}';
				if( isset( $luxe['head_band_google'] ) )	$style['all'] .= 'div[id*="head-band"] .google a{background:#dd4b39;}';
				if( isset( $luxe['head_band_youtube'] ) )	$style['all'] .= 'div[id*="head-band"] .youtube a{background:#ae3a34;}';
				if( isset( $luxe['head_band_rss'] ) )		$style['all'] .= 'div[id*="head-band"] .rss a{background:#fe9900;}';
				if( isset( $luxe['head_band_feedly'] ) )	$style['all'] .= 'div[id*="head-band"] .feedly a{background:#87bd33;}';
			}
		}
	}
...
UPWARD;
	...
	if( isset( $luxe['lazyload_enable'] ) ) {
		if( isset( $luxe['lazyload_placeholder'] ) || isset( $luxe['lazyload_spinner'] ) ) {
			$style['all'] .= 'img[data-lazy="1"] {background:';
			if( isset( $luxe['lazyload_placeholder'] ) ) {
				$style['all'] .= $luxe['lazyload_placeholder'];
			}
			if( isset( $luxe['lazyload_spinner'] ) ) {
...
FLUIDBOX;
	...
	if( isset( $luxe['add_external_icon'] ) ) {
		if( $luxe['external_icon_type'] !== 'normal' || isset( $luxe['external_icon_color'] ) ) {
...
add_action( 'get_header', 'thk_head_chk' );
...
WIDE_SIZE;
	...
	if( $luxe['side_1_width'] !== $defaults['side_1_width'] ) {
...
SIDE_POSITION;
		...
		if( $luxe['content_side_discrete'] === 'discrete' ) {
			...
			if( $luxe['container_max_width'] > 1170 ) {
...
#main {
	...
	-webkit-flex: 0 1 {$main_width_1310}px;
...
#side {
	...
	-webkit-flex: 0 0 {$side_1_width_wide}px;
...
DISCRETE;
		if( $column === '2column' ) {
			if( $luxe['container_max_width'] > 1170 || $luxe['container_max_width'] === 0 ) {
				$style['CONTAINER'] .= <<<WIDE_SIZE
#side .widget {
	margin: 0 0 15px 0;
	padding: 20px 32px;
...
COLUMN;
	...
	if( function_exists('dynamic_sidebar') === true ) {
		...
		if( $luxe['side_discrete'] === 'discrete' ) $style['min_992'] .= '#side-scroll .widget:first-child{border-top:1px solid ' . $side_border_color . ';}';
	}
