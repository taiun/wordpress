...
<h2 class="discussion"><i class="fa fa-id-card-o"></i><?php echo __( 'Discussion', 'luxeritas' ); ?></h2>
<?php
if( have_comments() ) {
...
?>
<h3 class="messages"><i class="fa fa-comments-o"></i><?php echo __( 'New Comments', 'luxeritas' ); ?></h3>
<ol class="comments-list" itemscope itemtype="http://schema.org/UserComments">
<?php
...
?>
<h3 class="messages"><i class="fa fa-reply-all"></i><?php echo __( 'New Pnigbacks &amp; Trackbacks', 'luxeritas' ); ?></h3>
<ol class="comments-list" itemscope itemtype="http://schema.org/UserComments">
<?php
		if( isset( $luxe['pings_reply_button'] ) ) {
	...
	elseif( $luxe['captcha_enable'] === 'securimage' ) {
		...
		$fields += array(
			'captcha' => '<p class="comment-form-captcha"><label for="captcha">' . __( 'CAPTCHA', 'luxeritas' ) . ' <span class="required">*</span></label> ' .
				     ...
				     '<a href="#" onclick="document.getElementById' . "('captcha')" . '.src = ' . "'" . TDEL . '/captcha.php?' . $get_pram . "&'" . ' + Math.random(); return false"><i class="fa fa-refresh" aria-hidden="true" style="font-size:32px;margin-left:10px;vertical-align:middle"></i></a></p>' .
...
$args = array(
	'fields' => $fields,
	'title_reply' => '<i class="fa fa-commenting-o"></i>' . __( 'Leave a Reply', 'luxeritas' ),
